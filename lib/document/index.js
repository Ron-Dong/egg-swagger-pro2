 ...
 * @param name
 */
const toHump = name => name.substring(1, name.length).replace(/\/(\w)/g, (_, letter) => letter.toUpperCase());
		...
		if (stat.isFile() && ['.js', '.ts'].indexOf(path.extname(name)) !== -1) {
			...
			for (let i = 1; i < blocks.length; i++) {
				...
				if (routers) {
					...
					const usePath = routers[0][2];
					const method = routers[0][1];
					path_method.operationId = generateOperationId(blocks[i]) || (toHump(usePath) + 'Using' + method.toUpperCase());

					if (!routerlist.includes(usePath)) {
						paths[usePath] = {};
					}

					routerlist.push(usePath);
					paths[usePath][method.toLowerCase()] = path_method;
 ...
 * @param {String} block 注释块
 */
function generateOperationId(block) {

	let operationId = '';
	let operationIds = comment.getComment(block, _.OPERATIONID);
	if (operationIds) {
		let m = 1;
		while (operationIds[0][m]) {
			operationId = operationId + operationIds[0][m];
			m++;
		}
	}
...
function generateDescription(block) {
	let description = '';
	let descriptions = comment.getComment(
	...
	if (responses) {
		for (let response of responses) {
			...
			if (response[3]) {
				if (!type) {
					throw new Error(
						`[egg-swagger-doc-pro] error at ${routers[0][1].toLowerCase()}: @response type(array or others) typeName Description, messing type`
					);
				}
				if (
					!['string', 'integer', 'number'].includes(response[3]) &&
					!definitions.hasOwnProperty(response[3])
				) {
					throw new Error(
						`[egg-swagger-doc-pro] error at ${routers[0][1].toLowerCase()}:${
							routers[0][2]
						} ,the type of response parameter does not exit`
					);
				}
				// 拓展response的定义，允许出现array格式
				if (type === 'array') {
					schema.type = 'array';
					schema.items = { $ref: `#/definitions/${response[3]}` };
					// 通过第三个参数判断实际使用类型（是否为string或number）
					if (response[3] === 'string') {
						schema.items = { type: 'string' };
					}
					if (['integer', 'number'].includes(response[3])) {
						schema.items = { type: 'integer', format: 'int32' };
					}
				} else {
					schema.$ref = `#/definitions/${response[3]}`;
					if (response[3] === 'string') {
						schema = { type: 'string' };
					}
					if (['integer', 'number'].includes(response[3])) {
